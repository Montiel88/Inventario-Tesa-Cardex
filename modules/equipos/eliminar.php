<?php
session_start();
if (!isset($_SESSION['user_id'])) {
    header('Location: /inventario_ti/login.php');
    exit();
}

// Verificar que sea admin
if ($_SESSION['user_rol'] != 'admin') {
    header('Location: listar.php?error=No tienes permisos para eliminar');
    exit();
}

require_once '../../config/database.php';

if (!isset($_GET['id'])) {
    header('Location: listar.php');
    exit();
}

$id = intval($_GET['id']);

// ============================================
// VERIFICAR SI EL EQUIPO TIENE ASIGNACIONES
// ============================================
$sql_check = "SELECT COUNT(*) as total FROM asignaciones WHERE equipo_id = $id AND fecha_devolucion IS NULL";
$result = $conn->query($sql_check);
$row = $result->fetch_assoc();

if ($row['total'] > 0) {
    // El equipo está prestado actualmente, no se puede eliminar
    header('Location: listar.php?error=No se puede eliminar un equipo que está prestado');
    exit();
}

// ============================================
// VERIFICAR SI TIENE MOVIMIENTOS
// ============================================
$sql_movimientos = "SELECT COUNT(*) as total FROM movimientos WHERE equipo_id = $id";
$result_mov = $conn->query($sql_movimientos);
$row_mov = $result_mov->fetch_assoc();

if ($row_mov['total'] > 0) {
    // Tiene movimientos, no se puede eliminar físicamente, mejor se marca como inactivo
    $sql_update = "UPDATE equipos SET estado = 'Dado de baja' WHERE id = $id";
    if ($conn->query($sql_update)) {
        header('Location: listar.php?mensaje=Equipo marcado como dado de baja (no se elimina por historial)');
    } else {
        header('Location: listar.php?error=Error al actualizar estado');
    }
} else {
    // No tiene movimientos, se puede eliminar completamente
    $sql_delete = "DELETE FROM equipos WHERE id = $id";
    if ($conn->query($sql_delete)) {
        header('Location: listar.php?mensaje=Equipo eliminado correctamente');
    } else {
        header('Location: listar.php?error=Error al eliminar: ' . $conn->error);
    }
}
exit();
?>