...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['realizar_traspaso'])) {
    ...
    if (!$asignacion_id || !$nueva_persona_id) {
        ...
        try {
            ...
            // ============================================
            registrarLog($conn, 'Traspaso equipo', "Asignación ID: {$asignacion_id} - Equipo ID: {$equipo_id} de Persona ID: {$persona_anterior_id} a Persona ID: {$nueva_persona_id}", $_SESSION['user_id']);
            // ============================================
