...
if (!isset($_SESSION['user_rol']) || $_SESSION['user_rol'] != 1) {
    header('Location: /inventario_ti/modules/equipos/listar.php?error=No tienes permisos');
    exit();
}

$equipo_id = intval($_GET['equipo_id'] ?? 0);
if ($equipo_id <= 0) {
    header('Location: /inventario_ti/modules/equipos/listar.php');
    exit();
}

// Verificar que el equipo existe
$stmt = $conn->prepare("SELECT tipo_equipo, codigo_barras FROM equipos WHERE id = ?");
$stmt->bind_param("i", $equipo_id);
$stmt->execute();
$result = $stmt->get_result();
if ($result->num_rows == 0) {
    header('Location: /inventario_ti/modules/equipos/listar.php?error=Equipo no encontrado');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nombre)) {
        ...
        if ($stmt->execute()) {
            // Registrar movimiento del componente (opcional, si tienes tabla de trazabilidad)
            // Por ahora solo redirigimos con mensaje
            header("Location: /inventario_ti/modules/equipos/detalle.php?id=$equipo_id&mensaje=Componente agregado correctamente");
            exit();
        } else {
        ...
        <div class="col-md-8 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0"><i class="fas fa-microchip me-2"></i>Agregar Componente</h4>
                    <p class="mb-0 text-muted">
                        Equipo: <strong><?php echo htmlspecialchars($equipo['tipo_equipo'] . ' - ' . $equipo['codigo_barras']); ?></strong>
                    </p>
                </div>
                <div class="card-body">
                    ...
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="especificaciones" class="form-control" rows="2" 
                                      placeholder="Ej: 600W, 80 Plus Bronze"><?php echo htmlspecialchars($_POST['especificaciones'] ?? ''); ?></textarea>
                        </div>
